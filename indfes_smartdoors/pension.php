...
<div id="mcaform">
    <div class="formbanner"><img src="pension.svg" width="100%" height="100%"></div>
    ...
    <td><input type="date" name="date"  style="padding-right=20px;" required></td></tr>
    <tr>
    <td><input type="text" name="gender"  placeholder="Gender" required></td>
    <td><select name="pension" id="mcareg">
             <option >Pension Type</option>
             <option value="wpension">Widow Pension</option>
             <option value="opension">Old Age Pension</option>
             <option value="depension">Physically Handicapped<br> Pension</option>
        </select></td>
    <td><input type="text" name="block"  placeholder="Tehsil/Block/Post Office" required></td></tr>
...
<option value=”West Bengal”>West Bengal</option></select>
    ...
    </td><td><input type="number" name="phone"  placeholder="Phone"></td>
...
if(isset($_POST['app_user_btn']))
{
  ...
  $query="INSERT INTO `user_form_pension`(pension_name,pension_f_h_name,pension_date,pension_gender,pension_service,pension_block,pension_district,pension_pincode,pension_state,pension_country,pension_phone,pension_email) VALUES('$name','$f_h_name','$date','$gender','$service','$block','$district','$pincode','$state','$country','$mobile','$email')";
  
  $result=mysqli_query($db,$query);  
  if($result>0)
  {
	echo "<script>window.location.assign('pension.php?msg=Your request is successfully submitted')</script>";
  }
  else
  {
	echo "<script>window.location.assign('pension.php?msg=Try again!!')</script>";
  }
  mysqli_close($db);
}
?>