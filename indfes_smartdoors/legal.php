...
<div id="mcaform">
    <div class="formbanner"><img src="legal.svg" width="100%" height="100%"></div>
    ...
    <td><input type="text" name="father_name"  placeholder="Father's Name" required></td>
    ...
    <td><input type="text" name="occupation"  placeholder="Occupation" required></td>
     ...
     <tr>
    <td><select name="legal" id="mcareg"><option >Select</option>
    <option value="lnotice">Legal Notice</option>
     <option value="cccr">Cyber Crime Complaint</option>
     <option value="civil">Civil Case Filing & Advice</option>
     <option value="criminal">Criminal Case Filing  & Advice</option>
     <option value="consumer">Consumer Case Filing & Advice</option>
     <option value="bhulekh">Land Record Verification</option>
     <option value="verification">Document Verification</option>
      <option value="nodues">No Dues Certificate for KCC</option>
     <option value="poa">Plan of Action</option>
     <option value="agreement">Rent or Lease Agreement</option>
     <option value="affidavit">Notarized Affidavit</option>
     <option value="aoa">Article of Association</option>
     <option value="moa">Memorandum of Association</option>
     <option value="consent">Notarized Consent Letter</option>
     </select></td>
...
if(isset($_POST['app_user_btn']))
{
  ...
  $query="INSERT INTO `user_form_legal`(legal_name,legal_father_name,legal_gender,legal_email,legal_phone,legal_occupation,legal_service,legal_state,legal_country) VALUES('$name','$father_name','$gender','$email','$phone','$occupation','$service','$state','$country') ";
  
  $result=mysqli_query($db,$query);  
  if($result>0)
  {
	echo "<script>window.location.assign('legal.php?msg=Your request is successfully submitted')</script>";
  }
  else
  {
	echo "<script>window.location.assign('legal.php?msg=Try again!!')</script>";
  }
  mysqli_close($db);
}
?>