...
<div id="mcaform">
    <div class="formbanner"><img src="taxation.svg" width="100%" height="100%"></div>
    ...
    <tr>
    <td><input type="text" name="addressa"  placeholder="Address 1" required></td>
    <td><input type="text" name="addressb"  placeholder="Address 2" required></td>
    <td><input type="text" name="city"  placeholder="City/Town" required></td>
    </tr>
    
    <tr>
    <td><select name="b_type" id="state">
    <option>Business Type</option>
    <option value=”inl”>Incorporation</option>
    <option value=”prop”>Proprietorship</option>
    <option value=”prop”>Partnership</option>
    <option value=”prop”>NGO / Section 8 Company</option>
    
    </select></td>
    <td><input type="text" name="b_name"  placeholder="Business or Trade Name" required></td>
    <td><input type="date" name="regd_date"  placeholder="Registration Date" required></td>
    </tr>
     ...
     <tr>
    <td><select name="accounting" id="mcareg"><option >Select</option>
     <option value="gstfile">GST Filing</option>
...
if(isset($_POST['app_user_btn']))
{
  ...
  $b_type=$_POST['b_type'];
  $b_name=$_POST['b_name'];
  $regd_date=$_POST['regd_date'];
  $service=$_POST['accounting'];
  $state=$_POST['state'];
  $country=$_POST['country'];
	
	include_once('config.php');
  
  $query="INSERT INTO `user_form_accounting`(account_name,account_email,account_phone,account_addressa,account_addressb,account_city,account_b_type,account_b_name,account_regd_date,account_service,account_state,account_country) VALUES('$name','$email','$phone','$addressa','$addressb','$city','$b_type','$b_name','$regd_date','$service','$state','$country')";
  
  $result=mysqli_query($db,$query);  
  if($result>0)
  {
	echo "<script>window.location.assign('accounting.php?msg=Your request is successfully submitted')</script>";
  }
  else
  {
	echo "<script>window.location.assign('accounting.php?msg=Try again!!')</script>";
  }
  mysqli_close($db);
}
?>