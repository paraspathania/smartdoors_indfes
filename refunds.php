...
.importlink{position: absolute;
	...
	.logsection #right .logarea .fields input#descript {
    width: 485px;
    height: 180px;
}
select#cars {
...
.logsection #right .logarea .fields input#issue {
    width: 530px;
    height: 150px;
}
label{
...
<h6>
<a href="index.php"><i class='bx bxs-home-smile' style='color:#2b27f9' ></i>Home</a><a href="login.php"><i class='bx bx-log-in'></i>Login</a></h6></div></div>
<!--Drop Down Menu!-->
<div class="menubar">
<div class="menub">
<!-- DROP DOWN LISTS!-->
...
<div id="left">
<div class="leftinfo">
<img src="refundhelp.svg" width="600px" height="500px">
</div></div>
<div id="right">
<div class="logarea">
...
?> </div>
<p id="Loginh">Refund's</p>
<div class="fields">
<form method="post">
<label>Name:<input type="text" id="username" name="name" value="<?php echo $_SESSION['customer'] ?>" disabled ></input>
<label>Email:<input type="text" id="username" name="email" value="<?php echo $_SESSION['email'] ?>" disabled ></input><br>
<label>Phone:<input type="text" id="username" name="phone" value="<?php echo $_SESSION['phone'] ?>" disabled ></input>
<label>Select:<select name="refund" id="cars"><br>
  <option value="refund">Refund</option>
</select><br>
<label>Type &nbsp;:<select name="types" id="cars"><br>
  <option value="c1">Refund Rejected</option>
  <option value="c2">Refund Request Pending</option>
  <option value="c3">Unable to place Refund </option>
  <option value="c4">Refund Not Receieved</option>
</select>
<label>ARN &nbsp;:<input type="text" id="username" name="arn_no" required></input><br>
<label>Issue&nbsp; :<input type="textbox" id="issue" name="issue" required></input></div>
<div class="log_button">
<button id="submit" name="refund_btn">Submit</button>
</form></div>
</div></div>
</div>
...
{ 
  ...
  $check_arn="SELECT * FROM refund_support where refund_email='$email' and arn_no='$arn' LIMIT 1";
  $result=mysqli_query($db,$check_arn);
  $user=mysqli_fetch_assoc($result);
  if($user)
  {
   if($user['arn_no']===$arn)
   echo "<script>window.location.assign('refunds.php?msg=You have already submitted your requested')</script>";
  }
  else
  {
  $query="INSERT INTO `refund_support`(refund_email,refund_pay,`query_types`,arn_no,issue_refund) values('$email','$refund','$types','$arn','$issue')";
  $result=mysqli_query($db,$query); 
  if($result>0)
  {
	echo "<script>window.location.assign('refunds.php?msg=Your request is successfully submitted')</script>";
  }
  else
  {
	echo "<script>window.location.assign('refunds.php?msg=Try again!!')</script>";
  }
}
}
?>