...
<div id="mcaform">
    <div class="formbanner"><img src="individual.svg" width="100%" height="100%"></div>
    ...
    <td><input type="text" name="f/hname"  placeholder="Father's Name / Husband Name" required></td>
    ...
    <td><input type="number" name="pincode"  placeholder="Pin Code" required></td>
    ...
    <td><select name="individual" id="mcareg"><option >Services</option>
     ...
     <option value="driving">Driving License</option></option>
     <option value="shram">Shram Card</option> 
     <option value="uan">UAN Card</option>
     <option value="vid">Voter ID</option></select></td>
     <td><select name="individual_action" id="mcareg"><option >Action</option>
...
if(isset($_POST['app_user_btn']))
{
  ...
  $action=$_POST['individual_action'];
  $email=$_SESSION['email'];
	
	include_once('config.php');
  
  $query="INSERT INTO `user_form_individual`(individual_name,individal_father,individual_mother,individual_gender,individaual_date,individual_phone,individual_houses,individual_district,individual_pincode,individual_service,individaul_action,individual_state,individual_country,email) VALUES('$name','$f_h_name','$mname','$gender','$date','$mobile','$landmark','$district','$pincode','$service','$action','$state','$country','$email')";
  
  $result=mysqli_query($db,$query);  
  if($result>0)
  {
	echo "<script>window.location.assign('inidividual.php?msg=Your request is successfully submitted')</script>";
  }
  else
  {
	echo "<script>window.location.assign('inidividual.php?msg=Try again!!')</script>";
  }
  mysqli_close($db);
}
?>